...
/// <summary>
/// Adds the <c>Authorization</c> header to outgoing trigger-client requests
/// based on the current <see cref="CroniqClientOptions"/>. <c>ApiKey</c>
/// takes precedence over <c>BearerToken</c> when both are set. Kept separate
/// from <see cref="CroniqAuthHandler"/> on purpose: the trigger client uses
/// its own credentials (<c>jobs:trigger</c> scope), never the runner's.
/// </summary>
internal sealed class CroniqClientAuthHandler(IOptionsMonitor<CroniqClientOptions> options) : DelegatingHandler
{
