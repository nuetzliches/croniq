...
- **A ceiling on consecutive authentication failures
  ([#473](https://github.com/nuetzliches/croniq/issues/473)).** New `RunnerOptions.max_consecutive_auth_failures` (default `3`, range `[1, 100]`) budgets
  consecutive `401 Unauthorized` responses to `POST /v1/work/poll`. On
  exhaustion the runner stops with the new `AuthFailedError`, which carries the streak length and
