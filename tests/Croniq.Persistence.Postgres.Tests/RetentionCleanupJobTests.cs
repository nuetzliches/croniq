...
public sealed class RetentionCleanupJobTests
{
    ...
    public RetentionCleanupJobTests(PostgresContainerFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task ExecuteAsync_deletes_expired_refresh_tokens_beyond_cutoff_for_current_tenant_only()
    {
        ...
        await using var provider = PostgresTestServiceProviderFactory.Create(_fixture.ConnectionString);
        var dbFactory = provider.GetRequiredService<IDbContextFactory<PostgresDbContext>>();
    ...
    private static async Task SeedTenantAsync(IDbContextFactory<PostgresDbContext> factory, string tenantId, CancellationToken cancellationToken)
    {
    ...
    private static async Task SeedRefreshTokenAsync(
        IDbContextFactory<PostgresDbContext> factory,
        string tokenId,
        string tenantId,
        string userId,
        string tokenHash,
    ...
    private sealed class TestExecutionContext : IJobExecutionContext
    {
        ...
        public ActivitySource ActivitySource { get; } = new("Croniq.Persistence.Postgres.Tests");
    }
