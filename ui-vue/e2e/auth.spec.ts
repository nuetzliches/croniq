...
test.describe('authentication', () => {
   ...
   *
   * The menu items are plain buttons inside a `role="menu"`, so `signOut`
   * (in `app.ts`) opens the menu before clicking — two steps rather than one
   * selector, which is why it does not sit inline here.
   */
