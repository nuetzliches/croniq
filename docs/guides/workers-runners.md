...
SDKs also emit an explicit disconnect heartbeat during drain/stop; the API interprets `transportState=disconnected`/`disconnectedAtUtc` to mark the runner offline immediately across transports.
