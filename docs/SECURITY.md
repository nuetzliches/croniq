...
- Signed container images (`croniq-api`, `croniq-worker`) published to GHCR with cosign.
...
- Containers: `cosign verify --key infra/signing/cosign.pub ghcr.io/<owner>/croniq-api:<tag>` (and `croniq-worker`). See `docs/deep-dive/release-verification.md` for step-by-step commands.
