...
- Deployment playbooks (Kubernetes, CI/CD)
...
- Aspire dev stack (`devstack.md`) - canonical source for AppHost usage, TLS setup, observability, and CI smoke references.
...
- `Croniq.Api` provides Minimal API endpoints:
  ...
  - `POST /jobs/trigger` (direct invocation via `IJobExecutionPipeline` when the job is registered locally; otherwise enqueues a one-off trigger)
