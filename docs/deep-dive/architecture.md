...
- **Activity Timeline & Summary**: `GET /tenants/{tenantId}/webhooks/activity` and `/tenants/{tenantId}/webhooks/activity/summary` expose recent delivery + dead-letter activity and bucketed counts for UI timelines/charts. Results are filtered by environment and optional `fromUtc`/`toUtc` window plus `hookKeys`/`jobKeys`. SqlServer/Postgres implementations merge `WebhookIngressEvents` with `WebhookDeadLetters` and annotate entries with `source` (`ingress` vs `invoke`) so manual invocations are distinguishable; the in-memory mode returns `503` so clients can fall back to polling/empty states.
- **Activity SLA**: Activity data is best-effort and eventually consistent; TriggerJob records successful dispatches (failures surface as dead letters), StoreOnly reflects relay acknowledgements, and the SSE stream polls every 5 seconds. Defaults: list `limit` 200 (max 500), summary window 24h with 60-minute buckets (max 31 days, max 24 hours).
- **UI Timeline Window**: The webhooks UI keeps a rolling 3-month activity window and refreshes incrementally using `updatedSinceUtc` to avoid full timeline reloads on every stream tick.
