...
Croniq can optionally schedule a maintenance job that prunes expired operational data from the SQL Server persistence store.
This is only wired when the worker host uses `Persistence.Mode = SqlServer`.
