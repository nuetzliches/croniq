...
| .NET (8, 10) | [`sdks/dotnet`](sdks/dotnet) | `Croniq.Runner.Sdk` + `Croniq.Runner.Sdk.OpenTelemetry` (NuGet) |
| Python (3.11+) | [`sdks/python`](sdks/python) | `croniq-runner` (PyPI) |
| Go (1.22+) | [`sdks/go`](sdks/go) | `github.com/nuetzliches/croniq/sdks/go` + `.../sdks/go/otel` (Go modules) |
| TypeScript / Node.js | [`sdks/typescript`](sdks/typescript) | `@nuetzliches/croniq-runner` (npm) |
| Java / Kotlin (JDK 21+) | [`sdks/java`](sdks/java) | `io.github.nuetzliches:croniq-runner` + `croniq-runner-spring-boot-starter` + `croniq-runner-kotlin-ext` + `croniq-runner-opentelemetry` (Maven Central) |
...
The Java SDK is JDK 21+, virtual-thread-first (Project Loom), with `java.net.http.HttpClient`, Jackson, and SLF4J as its only runtime deps. Ships in four modules: `croniq-runner` (core), `croniq-runner-spring-boot-starter` (Spring Boot 3 auto-config + `@CroniqJob` discovery + `croniq.runner.*` properties), `croniq-runner-kotlin-ext` (coroutine adapters + Kotlin DSL on top of the Java core), and `croniq-runner-opentelemetry` (opt-in observer with one span per execution). Published to Maven Central as of `v0.2.0`. See [`sdks/java/README.md`](sdks/java/README.md) for the quickstart.
...
ADD https://github.com/nuetzliches/croniq/releases/download/v0.39.1/croniq-x86_64-unknown-linux-musl.tar.gz /tmp/croniq.tar.gz
...
| `CRONIQ_API_CLIENT_<NAME>_SCOPES` | Scopes for that client, comma-separated (e.g. `work:poll,work:ack,work:renew`). **Required** — a named client with no scopes is a boot error, not an implicit `admin`. | — |
| `CRONIQ_API_KEY_RECONCILE` | Opt in to changing stored state from the environment: rotating a key, updating scopes, or taking ownership of a client that already exists as API-managed. Creating a client that does not exist yet happens without it. Default is to log the difference, so an accidental env change cannot silently revoke a working credential. | `0` |
| `CRONIQ_INIT_API_KEY` | **Deprecated alias** of `CRONIQ_API_KEY`, kept working for existing deployments. Also consumed by `croniq init` on a fresh data dir (Docker entrypoint). | — |
| `CRONIQ_INIT_API_KEY_RECONCILE` | **Deprecated alias** of `CRONIQ_API_KEY_RECONCILE`. | `0` |
| `CRONIQ_API_KEY_ROTATION_GRACE` | How long a key that a rotation replaced keeps working. The superseded key gets `expires_at = now + grace` instead of being revoked, so a runner that has not yet picked up the new value does not start failing the moment the server rotates. Accepts `<n>[s\|m\|h]` or bare seconds; a malformed value fails the boot. Set `0s` to revoke immediately (pre-v0.34 behaviour) — appropriate where a policy demands instant revocation. **Not the tool for a leaked key**: it deliberately keeps the old value alive. To kill one now, find its `key_id` with `GET /v1/api-keys?client_id=…` and call `DELETE /v1/api-keys/{id}`. | `15m` |
