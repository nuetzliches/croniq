...
| `CRONIQ_INIT_API_KEY` | Seed a default admin API client on first run. Must start with `croniq_` (e.g. `croniq_$(openssl rand -hex 32)`). Fails fast on first run if the prefix is missing. On every subsequent boot, the server compares this value against the stored key for client `default` and logs the outcome (match / differs / no default client). To **rotate** the key from the env var (installs the new one and retires existing active keys after `CRONIQ_API_KEY_ROTATION_GRACE`), additionally set `CRONIQ_INIT_API_KEY_RECONCILE=1`. See [issue #217](https://github.com/nuetzliches/croniq/issues/217). | — |
