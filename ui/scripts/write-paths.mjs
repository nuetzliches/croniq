// Drive the dashboard's write paths against a live dev stack.
//
// It began as a stopgap — the Playwright suite ran against the *built*
// dashboard and the rebuild had no e2e project of its own, so the screens that
// create, edit and delete things had no automated check at all. That gap is
// closed. What is left is still the only thing that walks every mutation in
// one pass against real data, which a smoke suite deliberately does not, so it
// stays.
//
// It reports any 4xx or page error made along the way:
//
...
await step("the DSL tab renders real Croniqfile syntax", async () => {
  ...
  // parse — see scripts/dsl-parses.mjs.
...
await step("the g chord works, unlike the hints it replaces", async () => {
  // The old palette printed `G D`, `G J` and the rest beside its entries
