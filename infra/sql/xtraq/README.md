...
- `croniq/types.sql`: Domain-UDTs (cronExpression, timeZoneId, stateCode, jobVariant, deadLetterReason) plus UDTTs `InstanceRef`, `JobRef`, `TriggerRef`, `TriggerLeaseRef`, `TriggerLeaseReleaseRef`, `TriggerDeadLetterRef` und Guard/Getter/Throw-Funktionen (z.B. `croniq.GuardInstanceRef`, `croniq.GuardJobRef`, `croniq.GuardTriggerRef`, `croniq.GuardTriggerLeaseRef`, `croniq.GuardTriggerLeaseReleaseRef`, `croniq.GuardTriggerDeadLetterRef`, `croniq.Throw*`).
...
- Reuse-Policy: Procs erwarten `@AllowDeletedReuse` (kein Default) fuer Upserts/Aquire; gefilterte Unique-Indizes (`WHERE IsDeleted = 0`) erlauben Reuse bei Soft-Delete.
- Error-Codes (5000x): Instance (`50001` not registered, `50002` ref incomplete, `50003` identity mismatch, `50005` reuse not allowed), Job (`50006` ref incomplete, `50007` not found, `50008` reuse not allowed), Trigger (`50009` ref incomplete, `50010` not found, `50011` reuse not allowed, `50012` job missing), TriggerLease (`50013` ref incomplete, `50014` release ref incomplete, `50015` active, `50016` not found, `50017` ownership mismatch, `50018` reuse not allowed), TriggerDeadLetter (`50019` ref incomplete), Actor (`50004` required).
