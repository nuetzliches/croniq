...
public sealed class CronExpression : ISerializable
{
    ...
    private void BuildExpression(string expression)
    {
        try
        {
            ...
            foreach (var token in tokens)
            {
                ...
                {
                    if (token.IndexOf('L') != -1 && token.Length > 1 && token.IndexOf(',') >= 0 && token.AsSpan(token.IndexOf('L') + 1).IndexOf('L') != -1)
                    {
                ...
                if (exprOn == CronExpressionConstants.DayOfWeek && token.IndexOf('L') != -1 && token.Length > 1 && token.IndexOf(',') >= 0)
                {
                    Throw.FormatException("Support for specifying 'L' with other days of the week is not implemented");
                }

                if (exprOn == CronExpressionConstants.DayOfWeek && token.IndexOf('#') != -1 && token.AsSpan(token.IndexOf('#') + 1 + 1).IndexOf('#') != -1)
                {
                ...
                if (token.IndexOf(',') != -1)
                {
                    foreach (var v in token.Split(',', StringSplitOptions.RemoveEmptyEntries))
                    {
                        StoreExpressionValues(0, v.AsSpan(), exprOn);
                    }
                }
                else
                {
                    // simple field
                    StoreExpressionValues(0, token.AsSpan(), exprOn);
                }
    ...
    internal CronField GetSet(int type)
    {
        ...
        if (field is null)
        {
            Throw.ArgumentOutOfRangeException<CronField>("Unexpected field type", nameof(type));
        }
