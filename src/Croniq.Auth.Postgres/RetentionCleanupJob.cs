...
public sealed class RetentionCleanupJob : IJob
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var deleted = await db.Database.ExecuteSqlInterpolatedAsync(
                $@"DELETE FROM ""croniq"".""DeadLetters"" dl
USING ""croniq"".""Triggers"" t, ""croniq"".""Jobs"" j
WHERE dl.""TriggerId"" = t.""Id""
  AND t.""JobId"" = j.""Id""
  AND j.""TenantId"" = {tenantId}
  AND j.""EnvironmentTag"" = {environmentTag}
  AND dl.""ExpiresAtUtc"" < {cutoffUtc};",
                cancellationToken)
            .ConfigureAwait(false);
